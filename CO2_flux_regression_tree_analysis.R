...
nee.seasonal.fit.plot <- ggplot(nee.seasonal.pred, aes(x = nee.sum, y = nee.pred)) +
  ...
  geom_text(x = -50, y = 175, label = nee.seasonal.r2.label,
            hjust = 0,
            vjust = 0,
            parse = TRUE) +
...
nee.seasonal.pd.biomass <- nee.seasonal.gbm %>%
  pdp::partial(pred.var = "biomass", n.trees = nee.seasonal.gbm$n.trees,
               grid.resolution = 100)
nee.seasonal.plot.1 <- plot.pdp(df1 = flux.seasonal, df2 = nee.seasonal.pd.biomass,
                                predictor = 'biomass', response = 'nee.sum',
...
nee.seasonal.pd.vwc.sd <- nee.seasonal.gbm %>%
  pdp::partial(pred.var = "vwc.sd", n.trees = nee.seasonal.gbm$n.trees,
               grid.resolution = 100)
nee.seasonal.plot.4 <- plot.pdp(df1 = flux.seasonal, df2 = nee.seasonal.pd.vwc.sd,
...
nee.seasonal.shap <- flux.seasonal[!is.na(nee.sum),
                                   c('flux.year',
                                     ...
                                     'tair.mean', 'tair.sd',
                                     'biomass')] %>%
  mutate(across(tp.annual:biomass, ~ (.x - mean(.x, na.rm = TRUE))/sd(.x, na.rm = TRUE))) %>%
  cbind.data.frame(nee.seasonal.shap) %>%
  select(-nee.sum.shap) %>%
  pivot_longer(cols = tp.annual:biomass.shap, 
               names_to = 'var', 
               values_to = 'value') %>%
...
reco.seasonal.fit.plot <- ggplot(reco.seasonal.pred, aes(x = reco.sum, y = reco.pred)) +
  ...
  geom_text(x = 110, y = 550, label = reco.seasonal.r2.label,
            hjust = 0,
            vjust = 1,
            parse = TRUE) +
...
reco.seasonal.pd.biomass <- reco.seasonal.gbm %>%
  pdp::partial(pred.var = "biomass", n.trees = reco.seasonal.gbm$n.trees,
               grid.resolution = 100)
reco.seasonal.plot.1 <- plot.pdp(df1 = flux.seasonal, df2 = reco.seasonal.pd.biomass,
                                predictor = 'biomass', response = 'reco.sum',
                                color.var = 'year.label', shape.var = 'treatment')
...
gpp.seasonal.fit.plot <- ggplot(gpp.seasonal.pred, aes(x = gpp.sum, y = gpp.pred)) +
  ...
  geom_text(x = 50, y = 675, label = gpp.seasonal.r2.label,
            hjust = 0,
            vjust = 1,
            parse = TRUE) +
...
gpp.seasonal.pd.biomass <- gpp.seasonal.gbm %>%
  pdp::partial(pred.var = "biomass", n.trees = gpp.seasonal.gbm$n.trees,
               grid.resolution = 100)
gpp.seasonal.plot.1 <- plot.pdp(df1 = flux.seasonal, df2 = gpp.seasonal.pd.biomass,
                                 predictor = 'biomass', response = 'gpp.sum',
                                 color.var = 'year.label', shape.var = 'treatment')
gpp.seasonal.plot.1
...
gpp.seasonal.pd.wtd <- gpp.seasonal.gbm %>%
  pdp::partial(pred.var = "wtd.mean", n.trees = gpp.seasonal.gbm$n.trees,
               grid.resolution = 100)
gpp.seasonal.plot.3 <- plot.pdp(df1 = flux.seasonal, df2 = gpp.seasonal.pd.wtd,
                                predictor = 'wtd.mean', response = 'gpp.sum',
                                color.var = 'year.label', shape.var = 'treatment')
gpp.seasonal.plot.3

gpp.seasonal.pd.spring.snow.depth <- gpp.seasonal.gbm %>%
  pdp::partial(pred.var = "spring.snow.depth", n.trees = gpp.seasonal.gbm$n.trees,
               grid.resolution = 100)
gpp.seasonal.plot.4 <- plot.pdp(df1 = flux.seasonal, df2 = gpp.seasonal.pd.spring.snow.depth,
                                 predictor = 'spring.snow.depth', response = 'gpp.sum',
                                 color.var = 'year.label', shape.var = 'treatment')
gpp.seasonal.plot.4
...
nee.monthly.pd.biomass <- nee.monthly.gbm %>%
  pdp::partial(pred.var = "biomass", n.trees = nee.monthly.gbm$n.trees,
               grid.resolution = 100)
nee.monthly.plot.2 <- plot.pdp(df1 = flux.monthly, df2 = nee.monthly.pd.biomass,
                                predictor = 'biomass', response = 'nee.sum',
                                color.var = 'month', shape.var = 'treatment')# +
  # geom_point(data = example.plots.monthly,
  #            aes(x = biomass, y = nee.sum),
  #            inherit.aes = FALSE,
  #            shape = 1, size = 3) +
  # geom_text(data = example.plots.monthly,
  #           aes(x = biomass, y = nee.sum, label = plot.id),
  #           inherit.aes = FALSE, size = 3, nudge_y = -7) +
  # geom_text(data = example.plots.monthly,
  #           aes(x = biomass, y = nee.sum, label = ID),
  #           inherit.aes = FALSE, size = 3, nudge_y = -15)
nee.monthly.plot.2
...
reco.monthly.fit.plot <- ggplot(reco.monthly.pred, aes(x = reco.sum, y = reco.pred)) +
  ...
  geom_text(x = 10, y = 170, label = reco.monthly.r2.label,
            hjust = 0,
            vjust = 0,
            parse = TRUE) +
  scale_x_continuous(name = expression('Measured Reco (gC m'^-2*')')) +
...
reco.monthly.pd.biomass <- reco.monthly.gbm %>%
  pdp::partial(pred.var = "biomass", n.trees = reco.monthly.gbm$n.trees,
               grid.resolution = 100)
reco.monthly.plot.2 <- plot.pdp(df1 = flux.monthly, df2 = reco.monthly.pd.biomass,
                               predictor = 'biomass', response = 'reco.sum',
                               color.var = 'month', shape.var = 'treatment')# +
  # geom_point(data = example.plots.monthly,
  #            aes(x = biomass, y = reco.sum),
  #            inherit.aes = FALSE,
  #            shape = 1, size = 3) +
  # geom_text(data = example.plots.monthly,
  #           aes(x = biomass, y = reco.sum, label = plot.id),
  #           inherit.aes = FALSE, size = 3, nudge_y = -7) +
  # geom_text(data = example.plots.monthly,
  #           aes(x = biomass, y = reco.sum, label = ID),
  #           inherit.aes = FALSE, size = 3, nudge_y = -15)
reco.monthly.plot.2
...
reco.monthly.pd.subsidence <- reco.monthly.gbm %>%
  pdp::partial(pred.var = "subsidence", n.trees = reco.monthly.gbm$n.trees,
               grid.resolution = 100)
reco.monthly.plot.4 <- plot.pdp(df1 = flux.monthly, df2 = reco.monthly.pd.subsidence,
                                predictor = 'subsidence', response = 'reco.sum',
                                color.var = 'month', shape.var = 'treatment') +
  # geom_point(data = example.plots.monthly,
  #            aes(x = subsidence, y = reco.sum),
  #            inherit.aes = FALSE,
  #            shape = 1, size = 3) +
  # geom_text(data = example.plots.monthly,
  #           aes(x = subsidence, y = reco.sum, label = plot.id),
  #           inherit.aes = FALSE, size = 3, nudge_y = -7) +
  scale_x_reverse(name = expression('Subsidence (cm)'),
                  breaks = seq(-100, 0, by = 25),
                  labels = seq(-100, 0, by = 25)*-1)# +
# geom_text(data = example.plots.monthly,
#           aes(x = subsidence, y = reco.sum, label = ID),
#           inherit.aes = FALSE, size = 3, nudge_y = -15)
reco.monthly.plot.4
...
gpp.monthly.pd.biomass <- gpp.monthly.gbm %>%
  pdp::partial(pred.var = "biomass", n.trees = gpp.monthly.gbm$n.trees,
               grid.resolution = 100)
gpp.monthly.plot.2 <- plot.pdp(df1 = flux.monthly, df2 = gpp.monthly.pd.biomass,
                                predictor = 'biomass', response = 'gpp.sum',
                                color.var = 'month', shape.var = 'treatment')# +
  # geom_point(data = example.plots.monthly,
  #            aes(x = biomass, y = gpp.sum),
  #            inherit.aes = FALSE,
  #            shape = 1, size = 3) +
  # geom_text(data = example.plots.monthly,
  #           aes(x = biomass, y = gpp.sum, label = plot.id),
  #           inherit.aes = FALSE, size = 3, nudge_y = -7) +
  # geom_text(data = example.plots.monthly,
  #           aes(x = biomass, y = gpp.sum, label = ID),
  #           inherit.aes = FALSE, size = 3, nudge_y = -15)
gpp.monthly.plot.2
...
gpp.monthly.pd.gdd.2m <- gpp.monthly.gbm %>%
  pdp::partial(pred.var = "gdd.2m", n.trees = gpp.monthly.gbm$n.trees,
               grid.resolution = 100)
gpp.monthly.plot.4 <- plot.pdp(df1 = flux.monthly, df2 = gpp.monthly.pd.gdd.2m,
                               predictor = 'gdd.2m', response = 'gpp.sum',
                               color.var = 'month', shape.var = 'treatment')# +
  # geom_point(data = example.plots.monthly,
  #            aes(x = gdd.2m, y = gpp.sum),
  #            inherit.aes = FALSE,
  #            shape = 1, size = 3) +
  # geom_text(data = example.plots.monthly,
  #           aes(x = gdd.2m, y = gpp.sum, label = plot.id),
  #           inherit.aes = FALSE, size = 3, nudge_y = -7) +
  # geom_text(data = example.plots.monthly,
  #           aes(x = gdd.2m, y = gpp.sum, label = ID),
  #           inherit.aes = FALSE, size = 3, nudge_y = -15)
gpp.monthly.plot.4
...
### Winter Estimates ###########################################################
flux.seasonal.filled <- fread('/home/heidi/Documents/School/NAU/Schuur Lab/Autochamber/autochamber_c_flux/input_data/flux_seasonal_filled_2019.csv')
...
flux <- read_rds('/home/heidi/Documents/School/NAU/Schuur Lab/Autochamber/autochamber_c_flux/input_data/flux_hh.RData')
flux[,
...
flux.seasonal.filled.winter <- flux.seasonal.filled[, .(flux.year, fence, plot, treatment, plot.id,
                         nee.sum, reco.sum, gpp.sum)]
flux.seasonal.filled.winter[, season := 1]
...
ggplot(flux.annual.filled.plotting,
       aes(x = flux.year, y = nee.sum.gs, color = subsidence, shape = factor(filled.gbm))) +
...
nee.trajectory.gs <- ggplot(flux.annual.filled.plotting,
       aes(x = flux.year, y = nee.sum.gs, color = biomass, shape = factor(filled.gbm))) +
  geom_hline(yintercept = 0) +
  geom_point() +
  geom_smooth(method = 'gam', formula = y ~ s(x, bs = "cs"), color = 'black') +
...
nee.trajectory <- ggplot(flux.annual.filled.plotting,
                            aes(x = flux.year, y = nee.sum.annual, color = biomass, shape = factor(filled.gbm))) +
  geom_hline(yintercept = 0) +
  geom_point() +
  geom_smooth(method = 'gam', formula = y ~ s(x, bs = "cs"), color = 'black') +
  scale_color_viridis(name = expression('Biomass (g m'^-2*')')) +
...
# Reco
ggplot(flux.annual.filled.plotting,
       aes(x = flux.year, y = reco.sum.gs, color = subsidence, shape = factor(filled.gbm))) +
  geom_hline(yintercept = 0) +
  geom_point() +
  geom_smooth(method = 'gam', formula = y ~ s(x, bs = "cs"), color = 'black') +
  scale_color_viridis(name = 'Subsidence (cm)') +
...
reco.trajectory.gs <- ggplot(flux.annual.filled.plotting,
       aes(x = flux.year, y = reco.sum.gs, color = biomass, shape = factor(filled.gbm))) +
  geom_hline(yintercept = 0) +
  geom_point() +
  geom_smooth(method = 'gam', formula = y ~ s(x, bs = "cs"), color = 'black') +
  scale_color_viridis(name = expression('Biomass (g m'^-2*')')) +
...
reco.trajectory <- ggplot(flux.annual.filled.plotting,
                          aes(x = flux.year, y = reco.sum.annual, color = biomass, shape = factor(filled.gbm))) +
  geom_hline(yintercept = 0) +
  geom_point() +
  geom_smooth(method = 'gam', formula = y ~ s(x, bs = "cs"), color = 'black') +
  scale_color_viridis(name = expression('Biomass (g m'^-2*')')) +
...
# GPP
ggplot(flux.annual.filled.plotting,
       aes(x = flux.year, y = gpp.sum.gs, color = subsidence, shape = factor(filled.gbm))) +
  geom_hline(yintercept = 0) +
  geom_point() +
  geom_smooth(method = 'gam', formula = y ~ s(x, bs = "cs"), color = 'black') +
  scale_color_viridis(name = 'Subsidence (cm)') +
...
gpp.trajectory.gs <- ggplot(flux.annual.filled.plotting,
       aes(x = flux.year, y = gpp.sum.gs, color = biomass, shape = factor(filled.gbm))) +
  geom_hline(yintercept = 0) +
  geom_point() +
  geom_smooth(method = 'gam', formula = y ~ s(x, bs = "cs"), color = 'black') +
  scale_color_viridis(name = expression('Biomass (g m'^-2*')')) +
...
gpp.trajectory <- ggplot(flux.annual.filled.plotting,
                            aes(x = flux.year, y = gpp.sum.annual, color = biomass, shape = factor(filled.gbm))) +
  geom_hline(yintercept = 0) +
  geom_point() +
  geom_smooth(method = 'gam', formula = y ~ s(x, bs = "cs"), color = 'black') +
  scale_color_viridis(name = expression('Biomass (g m'^-2*')')) +
...
flux.tk.time.series <- dcast(flux.tk.time.series[,
              ...
              flux.year + fence + plot + plot.id + treatment + tk.class.factor + 
                flux.type + filled.gbm + alt.annual + subsidence + 
                biomass + wtd.mean + wtd.sd + vwc.mean + vwc.sd + 
...
flux.tk.mean <- flux.tk[, .(nee.sum.gs = mean(nee.sum.gs, na.rm = TRUE),
                            ...
                            subsidence.mean = mean(subsidence),
                            subsidence.sd = sd(subsidence),
...
wtd.tk.class <- ggplot(flux.tk.mean, 
       ...
       size = 2) +
  annotate("rect", xmin = 0.3, xmax = 1.5, ymin = -Inf, ymax = Inf,
           alpha = .05) +
  geom_hline(yintercept = 0, linetype = 'dashed') +
  geom_jitter(data = flux.tk, 
              aes(x = tk.class.factor, y = wtd.mean*-1, color = pond), 
...
emmeans(wtd.sd.class.model, specs = pairwise~factor(tk.class),
        infer = c(TRUE, TRUE))
flux.tk.mean <- flux.tk.mean %>%
  mutate(wtd.sd.letters = c('ab', 'a', 'b', 'ab'))
...
wtd.sd.tk.class <- ggplot(flux.tk.mean, 
  ...
  annotate("rect", xmin = 0.3, xmax = 1.5, ymin = -Inf, ymax = Inf,
           alpha = .05) +
  geom_jitter(data = flux.tk, 
              aes(x = tk.class.factor, y = wtd.sd, color = pond), 
             inherit.aes = FALSE, size = 1, alpha = 0.5,
             width = 0.25) +
...
vwc.tk.class <- ggplot(flux.tk.mean, 
  ...
  annotate("rect", xmin = 0.3, xmax = 1.5, ymin = -Inf, ymax = Inf,
           alpha = .05) +
  geom_jitter(data = flux.tk, 
              aes(x = tk.class.factor, y = vwc.mean, color = pond), 
             inherit.aes = FALSE, size = 1, alpha = 0.5,
             width = 0.25) +
...
emmeans(vwc.sd.class.model, specs = pairwise~factor(tk.class),
        infer = c(TRUE, TRUE))
flux.tk.mean <- flux.tk.mean %>%
  mutate(vwc.sd.letters = c('ab', 'a', 'b', 'a'))
...
vwc.sd.tk.class <- ggplot(flux.tk.mean, 
  ...
  annotate("rect", xmin = 0.3, xmax = 1.5, ymin = -Inf, ymax = Inf,
           alpha = .05) +
  geom_jitter(data = flux.tk, 
              aes(x = tk.class.factor, y = vwc.sd, color = pond), 
             inherit.aes = FALSE, size = 1, alpha = 0.5,
             width = 0.25) +
...
gwc.tk.class <- ggplot(flux.tk.mean, 
  ...
  annotate("rect", xmin = 0.3, xmax = 1.5, ymin = -Inf, ymax = Inf,
           alpha = .05) +
  geom_jitter(data = flux.tk, 
              aes(x = tk.class.factor, y = gwc.mean, color = pond), 
             inherit.aes = FALSE, size = 1, alpha = 0.5,
             width = 0.25) +
...
emmeans(gwc.sd.class.model, specs = pairwise~factor(tk.class),
        infer = c(TRUE, TRUE))
flux.tk.mean <- flux.tk.mean %>%
  mutate(gwc.sd.letters = c('ab', 'a', 'b', 'a'))
...
gwc.sd.tk.class <- ggplot(flux.tk.mean, 
  ...
  annotate("rect", xmin = 0.3, xmax = 1.5, ymin = -Inf, ymax = Inf,
           alpha = .05) +
  geom_jitter(data = flux.tk, 
              aes(x = tk.class.factor, y = gwc.sd, color = pond), 
             inherit.aes = FALSE,  size = 1, alpha = 0.5,
             width = 0.25) +
...
emmeans(alt.class.model, specs = pairwise~factor(tk.class),
        infer = c(TRUE, TRUE))
flux.tk.mean <- flux.tk.mean %>%
  mutate(alt.letters = c('a', 'a', 'c', 'b'))

alt.tk.class <- ggplot(flux.tk.mean, aes (x = tk.class.factor, y = alt.mean*-1), size = 2) +
  annotate("rect", xmin = 0.3, xmax = 1.5, ymin = -Inf, ymax = Inf,
           alpha = .05) +
  geom_jitter(data = flux.tk, 
              aes(x = tk.class.factor, y = alt.annual*-1, color = pond), 
             inherit.aes = FALSE, size = 1, alpha = 0.5,
             width = 0.25) +
...
sub.class.model <- lm(subsidence ~ factor(tk.class),
                      data = flux.tk)
emmeans(sub.class.model, specs = pairwise~factor(tk.class),
        infer = c(TRUE, TRUE))
flux.tk.mean <- flux.tk.mean %>%
  mutate(sub.letters = c('a', 'a', 'c', 'b'))

subsidence.tk.class <- ggplot(flux.tk.mean, aes (x = tk.class.factor, y = subsidence.mean), size = 2) +
  annotate("rect", xmin = 0.3, xmax = 1.5, ymin = -Inf, ymax = Inf,
           alpha = .05) +
  geom_jitter(data = flux.tk, 
              aes(x = tk.class.factor, y = subsidence, color = pond), 
             inherit.aes = FALSE, size = 1, alpha = 0.5,
             width = 0.25) +
  geom_violin(data = flux.tk, aes(x = tk.class.factor, y = subsidence),
              color = 'gray40',
              fill = 'transparent') +
  geom_point() +
...
biomass.class.model <- lm(biomass ~ factor(tk.class),
                      data = flux.tk)
...
biomass.tk.class <- ggplot(flux.tk.mean, aes (x = tk.class.factor, y = biomass.mean), size = 2) +
  annotate("rect", xmin = 0.3, xmax = 1.5, ymin = -Inf, ymax = Inf,
           alpha = .05) +
  geom_jitter(data = flux.tk, 
              aes(x = tk.class.factor, y = biomass, color = pond), 
             inherit.aes = FALSE, size = 1, alpha = 0.5,
             width = 0.25) +
  geom_violin(data = flux.tk, aes(x = tk.class.factor, y = biomass),
              color = 'gray40',
              fill = 'transparent') +
  geom_point() +
...
### CO2 fluxes by tk class (each plot classified by year, 2010, 2017-2021)
# plot NEE
nee.class.model <- lm(nee.sum.gs ~ factor(tk.class),
                      data = flux.tk)
emmeans(nee.class.model, specs = pairwise~factor(tk.class),
        infer = c(TRUE, TRUE))
flux.tk.mean <- flux.tk.mean %>%
  mutate(nee.y = rep(400, 4),
         nee.sum.letters = c('a', 'b', 'a', 'b'))

nee.tk.class <- ggplot(flux.tk.mean, 
  ...
  geom_hline(yintercept = 0, linetype = 'dashed') +
  geom_jitter(data = flux.tk, 
              aes(x = tk.class.factor, y = nee.sum.gs, color = pond), 
              inherit.aes = FALSE, size = 1, alpha = 0.5,
              width = 0.25) +
  geom_violin(data = flux.tk, aes(x = tk.class.factor, y = nee.sum.gs),
              color = 'gray40',
              fill = 'transparent') +
  geom_point() +
  geom_errorbar(aes(ymin = nee.sum.gs - nee.sd.gs, ymax = nee.sum.gs + nee.sd.gs),
                width = 0.2) +
  geom_text(data = flux.tk.mean, 
            aes(x = tk.class.factor, y = nee.y, label = nee.sum.letters),
            inherit.aes = FALSE,
            size = 3) +
  scale_x_discrete(labels = c('Initial (Year 2)', 'Non-TK (Year 9-13)', 'TK Margin (Year 9-13)', 'TK Center (Year 9-13)')) +
  scale_y_continuous(name = expression('GS Flux (gC m'^-2*')')) +
...
# plot Reco
reco.class.model <- lm(reco.sum.gs ~ factor(tk.class),
                      data = flux.tk)
emmeans(reco.class.model, specs = pairwise~factor(tk.class),
        infer = c(TRUE, TRUE))
flux.tk.mean <- flux.tk.mean %>%
  mutate(reco.y = rep(50, 4),
         reco.sum.letters = c('a', 'b', 'b', 'ab'))

reco.tk.class <- ggplot(flux.tk.mean, 
                       aes (x = tk.class.factor, y = reco.sum.gs*-1), 
                       size = 2) +
  annotate("rect", xmin = 0.3, xmax = 1.5, ymin = -Inf, ymax = Inf,
           alpha = .05) +
  geom_hline(yintercept = 0, linetype = 'dashed') +
  geom_jitter(data = flux.tk, 
              aes(x = tk.class.factor, y = reco.sum.gs*-1, color = pond), 
              inherit.aes = FALSE, size = 1, alpha = 0.5,
              width = 0.25) +
  geom_violin(data = flux.tk, aes(x = tk.class.factor, y = reco.sum.gs*-1),
              color = 'gray40',
              fill = 'transparent') +
  geom_point() +
  geom_errorbar(aes(ymin = reco.sum.gs*-1 - reco.sd.gs, ymax = reco.sum.gs*-1 + reco.sd.gs),
                width = 0.2) +
  geom_text(data = flux.tk.mean, 
            aes(x = tk.class.factor, y = reco.y, label = reco.sum.letters),
            inherit.aes = FALSE,
            size = 3) +
  scale_x_discrete(labels = c('Initial (Year 2)', 'Non-TK (Year 9-13)', 'TK Margin (Year 9-13)', 'TK Center (Year 9-13)')) +
  scale_y_continuous(name = expression('GS Flux (gC m'^-2*')')) +
...
# plot GPP
gpp.class.model <- lm(gpp.sum.gs ~ factor(tk.class),
                       data = flux.tk)
emmeans(gpp.class.model, specs = pairwise~factor(tk.class),
        infer = c(TRUE, TRUE))
flux.tk.mean <- flux.tk.mean %>%
  mutate(gpp.y = rep(-50, 4),
         gpp.sum.letters = c('a', 'b', 'b', 'b'))

gpp.tk.class <- ggplot(flux.tk.mean, 
                        aes (x = tk.class.factor, y = gpp.sum.gs), 
                        size = 2) +
  annotate("rect", xmin = 0.3, xmax = 1.5, ymin = -Inf, ymax = Inf,
           alpha = .05) +
  geom_hline(yintercept = 0, linetype = 'dashed') +
  geom_jitter(data = flux.tk, 
              aes(x = tk.class.factor, y = gpp.sum.gs, color = pond), 
              inherit.aes = FALSE, size = 1, alpha = 0.5,
              width = 0.25) +
  geom_violin(data = flux.tk, aes(x = tk.class.factor, y = gpp.sum.gs),
              color = 'gray40',
              fill = 'transparent') +
  geom_point() +
  geom_errorbar(aes(ymin = gpp.sum.gs - gpp.sd.gs, ymax = gpp.sum.gs + gpp.sd.gs),
                width = 0.2) +
  geom_text(data = flux.tk.mean, 
            aes(x = tk.class.factor, y = gpp.y, label = gpp.sum.letters),
            inherit.aes = FALSE,
            size = 3) +
  scale_x_discrete(labels = c('Initial (Year 2)', 'Non-TK (Year 9-13)', 'TK Margin (Year 9-13)', 'TK Center (Year 9-13)')) +
  scale_y_continuous(name = expression('GS Flux (gC m'^-2*')')) +
...
tk.flux.class.plot <- ggarrange(gpp.tk.class +
                                  theme(axis.text.x = element_blank(),
                                ...
                                ncol = 1,
                                heights = c(0.55, 0.55, 1),
                                legend = 'top',
                                common.legend = TRUE)
tk.flux.class.plot
# ggsave('/home/heidi/Documents/School/NAU/Schuur Lab/Autochamber/autochamber_c_flux/figures/tk_class_flux.jpg',
#        tk.flux.class.plot,
#        height = 5.5,
#        width = 2.5,
#        bg = 'white')
# ggsave('/home/heidi/Documents/School/NAU/Schuur Lab/Autochamber/autochamber_c_flux/figures/tk_class_flux.pdf',
#        tk.flux.class.plot,
#        height = 5.5,
#        width = 2.5,
#        bg = 'white')
